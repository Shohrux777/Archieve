...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['sklad_delete_row']),
      for_edit(edit_data)
      {
        this.$router.push("/connect_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ArchiveRoomDetail/" + del_data.id, requestOptions);
