    ...
    <div class="datchik_add p-5">
      <form @submit.prevent="submit">
        <mdb-row>
         ...
         <mdb-row class="mt-4">
          ...
          <mdb-col col="5">
            ...
            <!-- <small class="invalid-text pt-4" style="margin-left:5px; "  v-if="$v.code.$dirty && !$v.code.required" >
              {{$t('name_invalid_text')}}
            </small>
        ...
        <mdb-row class="mt-4">
          ...
          <mdb-col col="5">
            <mdb-input class="m-0 p-0" v-model="min" size="md" outline  group type="number" validate error="wrong" success="right"/>
            <small class="invalid-text pt-4" style="margin-left:5px; "  v-if="$v.min.$dirty && !$v.min.required" >
              {{$t('name_invalid_text')}}
            </small>
          </mdb-col>
        </mdb-row> 
        <mdb-row class="mt-4">
          ...
          <mdb-col col="5">
            <mdb-input class="m-0 p-0" v-model="max" size="md" outline  group type="number" validate error="wrong" success="right"/>
            <small class="invalid-text pt-4" style="margin-left:5px; "  v-if="$v.max.$dirty && !$v.max.required" >
              {{$t('name_invalid_text')}}
            </small>
            <!-- <small class="p-0" style="margin-left:5px; font-size: 12px; color: gray;"  v-else >
              {{$t('write_company_name_to_chek')}}
...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/ArchiveDatchik/' + this.id);
  ...
  methods:{
    ...
    save_data :  async function(){
        ...
        const requestOptions = {
          ...
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/ArchiveDatchik", requestOptions);
    ...
    async submit_continue()
      {
        ...
        {
          if(this.id != 0){
            this.$router.push('/datchik_add/0')
            this.cls_wnd();
          }
          else{
            this.cls_wnd();
