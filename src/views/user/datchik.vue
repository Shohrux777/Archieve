...
export default {
  ...
  methods: {
    ...
    ...mapMutations(['client_delete_row']),
      for_edit(edit_data)
      {
        this.$router.push("/datchik_add/" + edit_data.id);
      },
      async for_delete(del_data,index)
      {
         const requestOptions = {
          ...
          const response = await fetch(this.$store.state.hostname + "/ArchiveDatchik/" + del_data.id, requestOptions);
