...
export default {
  ...
  validations: {
    ...
    async created()
    {
      if(this.id > 0)
      {
        const res = await fetch(this.$store.state.hostname + '/ArchiveDepartment/' + this.id);
    ...
    save_data :  async function(){
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({
              ...
              "archiveCompanyid": this.comp_id,
              "id": this.id
            })
          };
          try{
            this.loading = true;
            const response = await fetch(this.$store.state.hostname + "/ArchiveDepartment", requestOptions);
